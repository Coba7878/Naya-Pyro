...
async def pmpermit(client, message):
    ...
    x = await client.get_inline_bot_results(app.me.username, f"pmpermit {int(org)}")
...
async def pmpermit_func(message, answers):
    ...
    siapa = message.from_user.mention
...
async def inline_query_handler(client, inline_query):
    try:
        ...
        get_id = int(inline_query.query.split())
        answers = []
        if string.split()[0] == "pmpermit":
            m = [obj for obj in get_objects() if id(obj) == get_id][1]
